...
//A클래스는 equals / hashcode를 오버라이드 하지 않음
class A{
    int data;
    public A(int data){
        this.data=data;
...
//B클래스는 equals만 오버라이드하고 hashcode는 오버라이드 하지 않음
class B{
    int data;
    public B(int data){
        this.data =data;
...
//C클래스는 equals / hashcode를 오버라이드 모두 오버라이드
class D{
    int data1;
    int data2;
    ...
    @Override
    public int hashCode() {
        //hash메서드는 무자열이나 숫자 무엇이든 매개변수로 넣으면 함께 계산
        //동일한 데이터들을 매개변수로 전달하면 항상 동일한 결과를 리턴
        //이러한 hash의 특성 때문에 로그인 시 패스워드의 인증에 사용됨
...
public class HashSet_2 {
    public static void main(String[] args) {
        ...
        hset1.add(a1);
        hset1.add(a2);
        //a1,a2객체의 equals는 false,hashcode를 다르므로 중복 아님
        System.out.println(hset1.size());//2
        System.out.println();
        //2. B 클래스(equals만 오버라이드)
        ...
        hset2.add(b1);
        hset2.add(b2);
        //equals는 true였지만 hashcode가 다르기 때문에 중복 아님(서로 다른 객체이다)
        System.out.println(hset2.size());//2
        System.out.println();
        //3. C클래스(equals/hashcode 오버라이드)
        ...
        //4. D클래스(equals/hashcode 오버라이드 후 매개 변수 2개)
        D d1 =new D(5,6);
        D d2 =new D(5,6);
