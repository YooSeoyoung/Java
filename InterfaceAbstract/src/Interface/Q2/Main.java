...
// 기본 도형 클래스
abstract class Shape implements Calculatable, Colorable { //도형의 계산 기능 이랑 색상 변경 기능
...
class Triangle extends Shape {
    ...
    public Triangle(double base, double height, String color) {
        super(color); //-> shape 클래스의 필드
        this.base = base;
        this.height = height;
    }

    @Override
    public double getArea() { //(밑변 * 높이 / 2)
...
// 원 클래스
class Circle extends Shape implements Resizable { // 크기 조절이 가능한 기능
    private double radius;
    private static final double PI = 3.14159; //파이
...
class Square extends Shape implements Resizable {
    ...
    @Override
    public double getPerimeter() { //정사각형의 둘레를 계산
       return side*4;
    }
}
public class Main {
